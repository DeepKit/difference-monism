...
本文基于 Paper I 中的合法性定义和 Paper II 中的契约获取机制，研究如何通过受控的契约精准度干预和多智能体对抗验证来执行产出物合法性。
...
2. **刚性化风险**：过度形式化的契约可能抑制探索性编程和原型设计阶段的灵活性。
2. **领域局限性**：ODD 不适用于需求本质上无法预知的创造性编程（如艺术生成、开放式对话）。
3. **权力集中风险**："契约架构师"角色可能成为组织的新瓶颈，需要配套的能力建设与分权机制。
4. **数据验证状态**：当前假设基于理论推导与小规模试点，大规模生产数据验证尚在进行中。
...
**核心洞见**：控制 AI 输出质量最有效的杠杆不是模型，而是契约。

### 9.1 后续工作

本文描述的机制在给定时间点执行合法性，但并未解决合法性标准本身如何演化的问题。随着系统规模扩大和上下文变化，静态的执行标准将变得不足。

合法性随时间演化是 **Paper IV**（*AI原生软件组织的合法性演化与治理*）的主题。

支撑契约精准度和对抗验证的上下文工程基础设施详见 **Paper S1**（*面向可审计LLM工作流的上下文工程*）。
