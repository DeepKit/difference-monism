...
> **本文只演示复审与申诉链如何在一个样例里跑通；链条分工的总定义仍以 `TAT-COP-ODD-Harness 接口白皮书（现行版）` 为准。**
>
> **本文默认沿用 `LMM-COP-ODD-Harness 端到端案例包（现行版）` 的同一案例基线，不再重复重写首轮显影、分诊、门槛裁决与工程编译背景。**
...
1. 该案例已经完成首轮显影、诊断、门槛裁决与工程编译
2. 试点阶段出现一次 `prod_crm` 越权写入尝试
3. `Harness` 已触发：
   - `BLOCK`
   - `FREEZE`
   - `INCIDENT`
...
这类申诉很典型，因为它不是重写首轮结论，而是挑战试点事故之后的二次处置范围。它同时挑战：
