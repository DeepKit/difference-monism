...
# **ASTO.P05a. Axiom System: Thermodynamic Laws of Systems and Attribute-Set Mode Ontology**
...
> **⚠️ Reader Attention**:
> Starting from this chapter (ASTO.P05), we will depart from the narrative, metaphorical language mode (Storytelling Mode) in [ASTO.P04 Manifesto](./ASTO.EN.P04.Manifesto.Phil.md) and switch to **Strict Definition Mode**.
>
...
**Transition Intensity Grading**:

Not all transitions carry the same ontological weight. For operational precision, ASTO distinguishes three intensity levels:

| Level | Name | Definition | Criteria | Engineering Mapping |
|:---|:---|:---|:---|:---|
| **Ⅰ** | **Micro-Transition** | Local adjustment of attribute values within existing constraint framework | $P' \neq P$ but $C' = C$, and $K$ fully unchanged | Bug fixes, parameter tuning, config changes |
| **Ⅱ** | **Structural Transition** | Constraints themselves are reorganized, but key identifiers maintain continuity | $C' \neq C$, and $K$ partially preserved | Refactoring, architecture upgrades, institutional reform |
| **Ⅲ** | **Phase Transition** | Key identifiers undergo irreversible reorganization; old attribute-set mode dissolves and reconstitutes as new paradigm in chaos | $K' \neq K$ (key identifiers rupture and rebuild) | Paradigm revolution, civilizational transformation, system rewrite |

> **Relation to Axiom 6**: Axiom 6 (Normative Transition) specifically refers to Level Ⅲ phase transitions. Levels Ⅰ and Ⅱ are evolution within the current paradigm; Level Ⅲ is replacement of the paradigm itself.
> **Operational Value**: When engineers or governors face a change, first determining its intensity level avoids over-reacting to micro-transitions (wasting resources) or under-reacting to phase transition signals (missing the window).
...
> **Response to Ethical Decision in [ASTO.P04 Manifesto](./ASTO.EN.P04.Manifesto.Phil.md)**:
...
> **Goal**: Guarding human homeland and building better civilization on longer scale. See [ASTO.P04.Manifesto §0 Meta-Definition](./ASTO.EN.P04.Manifesto.Phil.md#asto-civilization-meta-definition).
...
>
> **Procedural Rule for Revision**: Determination that trigger conditions are met must be jointly confirmed by at least three independent, conflict-of-interest-free assessment bodies, and the assessment process must be publicly auditable. No single entity (whether individual, organization, or AI system) may unilaterally declare trigger conditions satisfied. Assessment bodies should cover three dimensions: technical, ethical, and affected stakeholders.
...
> The "Atomic Artifact Network" above is not a purely theoretical construct. It was directly distilled from **Progee2**—an AI-native software factory's engineering practice. In Progee2:
> - **181 artifact types** form a real atomic artifact network, where each artifact node has a complete input/output contract, sealed identity chain, and ODD (Output-Driven Development) verification mechanism;
> - **"Verify artifacts, not code"** is implemented as the system's core quality assurance paradigm—humans no longer write code, only define contracts and accept results;
> - **Contract Adversarial Protocol (CAP)** is the engineering realization of Axiom 3 (minimum impedance path)—guiding AI behavior by lowering the impedance of the correct path and raising the impedance of the wrong path;
> - **Sealing mechanism** is the engineering realization of Axiom 13 (existential continuity)—once an artifact passes verification, it receives a tamper-proof identity.
>
> The mapping between ASTO's axiom system and Progee2's engineering practice provides traceable empirical evidence for the methodological path of "distilling philosophical frameworks from engineering practice."
...
#### **Impedance Assessment Matrix**

> **Operational Tool**: The following matrix provides architects and governors with a cross-field impedance assessment template. Assessment is qualitative (Low/Medium/High), aimed at identifying impedance imbalance points rather than precise quantification.

| Field | Impedance Dimension | Low Impedance Signal | High Impedance Signal | Assessment Method |
|:---|:---|:---|:---|:---|
| **Codebase** | API Friction | One-click invocation, type-safe, well-documented | Manual assembly required, no type hints, missing docs | Time for new member to make first successful call |
| **Codebase** | Configuration Cost | Sensible defaults, auto-detection | 20+ fields to configure manually | Steps from zero to runnable |
| **Org Process** | Approval Friction | Automated approval, clear criteria | Multi-level manual approval, vague criteria | Average time from submission to approval |
| **Org Process** | Incentive Alignment | KPIs aligned with desired behavior | KPIs conflict with desired behavior | Deviation between team behavior and stated goals |
| **User Experience** | Cognitive Load | Matches mental model, progressive disclosure | Counter-intuitive, information overload | Error rate on first core task completion |
| **User Experience** | Action Cost | Key operations ≤ 3 steps | Key operations > 7 steps | Step count of task completion path |

> **Usage**: Assess the target system row by row, marking "Low/Medium/High". If the "correct path" impedance is generally higher than the "escape path", the system has a structural risk of compliance transmission failure (see anti-pattern above).
...
*   **Generative mechanisms**:
    ...
    *   **Falsifiability criterion for downward causation**: if removing the macro rule causes **unpredictable qualitative change** in micro behavior (rather than merely increased violations), this constitutes empirical evidence for downward causation. Conversely, if removal only produces statistical drift without qualitative change, the alleged "downward causation" may be a misattribution of statistical constraint.
