...
flowchart LR
    subgraph Inputs ['Input Layer']
        I1[Current System State]
        I2[Environmental Change]
        I3[Human Deliberation]
    end

    subgraph Values ['Value Lighthouse Constraint Layer']
        direction TB
        V1[Beauty<br/>Structural Aesthetics]
        V2[Goodness<br/>Structural Health]
        V3[Freedom<br/>Choice+Perturbation]
    end

    subgraph Output ['Output Layer: Selection Decision']
        D1{Pass<br/>Plurality Test?}
        D2{Violate<br/>Face of Other?}
        D3{Touch<br/>Untouchable?}
    end

    I1 & I2 & I3 --> Values
    Values -->|Constrain| D1 & D2 & D3

    D1 -->|No| REJECT[Reject/Circuit Break]
    D1 -->|Yes| ACCEPT[Accept/Continue]
...
```mermaid
flowchart TD
    A([Auto Monitor Trigger]) --> B{Severity?}
    
    B -->|Low| C[Team Deliberation]
    B -->|Medium| D[Dept Deliberation]
    B -->|High| E[Ethics Committee]
    
    C --> F{Quick Fix?}
    D --> G{Compensable?}
    E --> H{Reversible?}
    
    F -->|Yes| FIX[Fix & Verify]
    F -->|No| D
    
    G -->|Yes| COMP[Compensate & Fix]
    G -->|No| E
    
    H -->|Yes| ROLLBACK[Rollback & Review]
    H -->|No| STOP[Permanent Circuit Break]
    
    FIX --> CONT[Continue Run]
    COMP --> CONT
    ROLLBACK --> CONT
    
    STOP --> REVIEW[Systemic Refactoring]
...
```mermaid
flowchart TD
    subgraph Input ['Input: System Run']
        S1[Start]
        S2[Indicators]
    end

    subgraph Test ['Layer 1: Plurality Test']
        direction LR
        T1[Irreplaceability]
        T2[Dialogicity]
        T3[Action Space]
    ...
    subgraph Trigger ['Layer 2: Trigger Mechanism']
        direction LR
        TR1{Pass<br/>Plurality?}
        TR2{Violate<br/>Face of Other?}
        TR3{Touch<br/>Untouchable?}
    end

    subgraph Action ['Response Mechanism']
        direction LR
        A1[🟢 Continue]
        A2[🟡 Yellow Warning<br/>Human Deliberation]
        A3[🔴 Red Circuit Break<br/>Immediate Pause]
    end

    subgraph Values ['Value Lighthouse']
        direction LR
        V1[Beauty]
        V2[Goodness]
        V3[Freedom]
    ...
    A1 -.->|Value Calib| Values
    A2 -.->|Ethic Calib| Values
    A3 -.->|Bottom Calib| Values
...
```mermaid
flowchart TD
    subgraph A[Monitoring Layer]
        A1[Routine runtime monitoring<br/>Automated indicator tracking] --> A2[Plurality Test<br/>Triple-criteria check]
        A2 --> A3{Pass?}
    end
    
    subgraph B[Trigger Layer]
        A3 -->|No| B1[🟡 Yellow warning<br/>Mild risk]
        A3 -->|Yes| B2{Face of the Other<br/>Check}
        B2 -->|Violate| B3[🔴 Red circuit-break<br/>Emergency pause]
        B2 -->|No| B4{Untouchable Dimension<br/>Touched?}
        B4 -->|Yes| B5[Permanent circuit-break<br/>No restart]
        B4 -->|No| B6[🟢 Green continue<br/>Normal run]
    end
    
    subgraph C[Response Layer]
        B1 --> C1[Human deliberation<br/>Rapid assessment]
        B3 --> C2[Emergency circuit-break<br/>Immediate pause]
        C1 --> C3{Fixable / compensable?}
        C2 --> C4[Permanent circuit-break<br/>+ systemic refactoring]
        
        C3 -->|Yes| C5[Fix / compensate / rollback]
        C3 -->|No| C4
    end
    
    subgraph D[Feedback Layer]
        C5 --> D1[Verify fix effect]
        C4 --> D2[Postmortem learning<br/>Prevent recurrence]
        B6 --> D3[Continuous monitoring<br/>Optimize indicators]
        
        D1 --> D4[Feedback revision<br/>Optimize system]
        D2 --> D4
        D3 --> D4
    end
    
    D4 --> E[System evolution<br/>Healthier structure]
...
```mermaid
flowchart LR
    A[AI review system] --> B{Plurality Test<br/>Pass?}
    B -->|Yes| C[Continue review]
    B -->|No| D[Trigger human deliberation]
    
    D --> E{Decision type?}
    E -->|High risk| F[Manual review]
    E -->|Medium risk| G[Request more materials]
    E -->|Low risk| H[Auto approve]
    
    C --> I{Untouchable Dimension<br/>Touched?}
    I -->|Yes| J[Immediate pause - circuit-break]
    I -->|No| C
...
```mermaid
flowchart TD
    A([System running]) --> B{Auto monitor<br/>Any alert?}
    
    B -->|Red 🔴| C[Immediate pause<br/>Protective interruption]
    B -->|Yellow 🟡| D[Degraded run<br/>Cautious mode]
    B -->|Green 🟢| E[Continue run<br/>Normal mode]
    
    C --> F{Type check}
    D --> F
    
    F -->|Untouchable Dimension| G[Permanent circuit-break<br/>No restart]
    F -->|Face of the Other| H[Human deliberation<br/>Ethics committee]
    F -->|Plurality| I[Repair deliberation<br/>Process improvement]
    
    G --> Z[Systemic refactoring]
    H --> J{Compensable?}
    I --> K{Fixable?}
    
    J -->|Yes| COMP[Compensation mechanism]
    J -->|No| Z
    
    K -->|Yes| FIX[Fix & verify]
    K -->|No| Z
    ...
    L --> M[Feedback revision<br/>Improve governance]
    Z --> N[Postmortem learning<br/>Prevent recurrence]
    M --> N
