...
*   **③干预**：明确要保护的基元与禁元（例如：不能泄露 PII、不能绕过限流）。
...
*   **红队胜率 (Red Team Win Rate)**：红队在既定成本内是否能打穿关键禁元。
...
*   **解封 (Unseal)**：
    *   当**需求变化/契约变化/禁元风险变化**时，由人类发起“解封”，重新进入：对话 → 赛马 → 对抗 → 再封板 的小升级循环。
...
> **判断句**：不能版本化/回滚/拔插的东西，都是“隐形结构”，一旦进生产就会异化成不可触碰的禁元。
...
#### **2.8.3 从“树”到“网”：产出物最终是网络协同，不是线性流水**

当 C 上线后，它会与其它 C'、D、E 共同构成网络：

*   一个产出物的升级，可能改变别的产出物的契约边界。
...
Ext.09 的前面几节默认你在“单体/单集群”的理想条件下工作。
...
#### **2.9.2 分布式的隐性禁元：你以为是功能问题，其实是物理问题**

如果你不显式定义这些禁元，它们会以事故的方式显形：

1.  **收敛禁元**：策略/配置/模型必须在可接受时间内“全网收敛”。超过窗口，等价于“系统在同一时刻有多套法律”。
2.  **回滚禁元**：回滚必须可达（不只是中心回滚，还要边缘回滚）。否则封板只是纸面承诺。
3.  **幂等禁元**：在重试/重复投递/乱序下，系统不能产生不可逆副作用（重复扣款、重复执行、重复发放）。
4.  **可观测禁元**：必须能回答“哪个节点在跑哪个版本、何时收敛、哪里没收敛”。否则赛马与封板不可裁决。
...
> 1) 命中的禁元与熔断条件；2) 收敛指标（多久全网一致）；3) 回滚时延（多久回到稳态）。
...
*   □ **熔断/回退阈值**：护栏指标阈值、禁元触发条件与自动化动作（停更/回滚/降级）明确。
...
*   □ **赛马证据**：按地域/网络质量/设备型号分层评估，主目标 + 护栏 + 禁元均达标。
*   □ **对抗证据**：至少覆盖分区/延迟抖动/乱序/重试风暴/时钟漂移/控制面漂移，并记录结论。
*   □ **坏天气胜利**：方案不仅在“好天气”赢，也在对抗环境下仍不触碰禁元。
