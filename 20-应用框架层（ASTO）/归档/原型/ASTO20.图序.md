...
*   **来源**：`ASTO12.Web3` (原 ASTO10; 重构为 Mermaid)
*   **描述**：展示了人类意图在编译过程中如何丢失 NEN 属性。
...
## **图 09：规范可执行性梯度 (NEG)**
*   **来源**：`ASTO11.自动化` (原 ASTO09; 重构为 Mermaid)
*   **描述**：展示了从 L1 到 L4 的自动化分级。
...
```mermaid
graph TB
    subgraph Civilization [ASTO 文明元定义]
        direction TB
        
        Goal((反脆弱生态))
        
        subgraph Pillars [三大支柱]
            P1[结构支撑自由<br/>(Structure as Support)]
