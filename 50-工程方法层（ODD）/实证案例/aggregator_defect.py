...
class Aggregator:
    ...
    def aggregate(self, **operations) -> Dict[str, Any]:
        """自定义聚合操作"""
        results = {}
        for name, func in operations.items():
            results[name] = func(self.data)
        return results
...
if __name__ == "__main__":
    ...
    agg2 = Aggregator().add_many(data)
    agg2.group_by(lambda x: x["category"])
    grouped_sums = agg2.group_aggregate(lambda items: sum(item["value"] for item in items))
    print(f"分组求和: {grouped_sums}")