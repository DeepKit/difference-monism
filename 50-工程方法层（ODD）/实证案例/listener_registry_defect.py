...
if __name__ == "__main__":
    ...
    class MyListener(Listener):
        ...
        def on_event(self, event_type: str, data: Any) -> None:
            print(f"{self.name} 收到事件 [{event_type}]: {data}")
    ...
    registry.register("data_changed", listener1)
    registry.register("data_changed", listener2)
    
    # 注册回调函数
    registry.register_callback("data_changed", 
        lambda data: print(f"回调函数收到: {data}"))
    
    # 触发事件
    registry.notify("data_changed", {"value": 100})
    
    # 注销监听器
    registry.unregister("data_changed", listener1)
    
    # 再次触发
    print("\n注销listener1后:")
    registry.notify("data_changed", {"value": 200})