...
class CreateUserHandler(CommandHandler):
    ...
    def handle(self, command: CreateUserCommand) -> str:
        ...
        user.add_event(UserCreated(user_id=user_id, email=command.email, name=command.name))
...
class GetUserHandler(QueryHandler):
    ...
    def handle(self, query: GetUserQuery) -> Optional[UserDTO]:
        user = self.read_repository.get(query.user_id)
        if not user:
            return None
...
class ListUsersHandler(QueryHandler):
    ...
    def handle(self, query: ListUsersQuery) -> List[UserDTO]:
        users = self.read_repository.list(query.limit, query.offset)
...
from example.queries import GetUserQuery, GetUserHandler, ListUsersQuery, ListUsersHandler
from example.repositories import UserWriteRepository, UserReadRepository
...
def main():
    ...
    command_bus.register(CreateUserCommand, CreateUserHandler(write_repo))
    command_bus.register(UpdateUserCommand, UpdateUserHandler(write_repo))
    
    # Setup query bus
    query_bus = QueryBus()
    query_bus.register(GetUserQuery, GetUserHandler(read_repo))
    query_bus.register(ListUsersQuery, ListUsersHandler(read_repo))
    
    # Create user
    user_id = command_bus.execute(CreateUserCommand(email="user@example.com", name="John Doe"))
    print(f"Created user: {user_id}")
    
    # Sync to read model (in real app, use event handlers)
    user = write_repo.get(user_id)
    read_repo.sync(user)
    
    # Query user
    user_dto = query_bus.execute(GetUserQuery(user_id=user_id))
    print(f"Retrieved user: {user_dto}")
    ...
    # Query updated user
    user_dto = query_bus.execute(GetUserQuery(user_id=user_id))
    print(f"Updated user: {user_dto}")
    
    # List users
    users = query_bus.execute(ListUsersQuery(limit=10))
