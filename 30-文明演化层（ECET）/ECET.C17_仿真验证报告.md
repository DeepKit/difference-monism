...
subtitle: "基于智能体模型（ABM）的 H1a、H2a、H4a 概念验证级支撑"
date: "2026-02-25"
version: "1.0"
...
当前版本不再把 `C17` 直接表述为对 `B09` 正式主假设 `H1 / H2 / H4` 的完成验证，而是固定为对三个**概念验证子假设**的支撑：

| `C17` 当前对象 | `B09` 中的位置 | 当前口径 |
|---------------|----------------|---------|
| `H1a` | `H1` 的形态支持子假设 | 支持“存在最优偏差/约束区间”的早期形态判断 |
| `H2a` | `H2` 的探索性子假设 | 支持“零容忍回退并非总是最优”的动态环境判断 |
| `H4a` | `H4` 编号谱系中的早期子假设 | 支持“不完备性可在一定区间内成为探索资源”的早期机制判断 |

因此，`C17` 的最严格公开口径应固定为：**它为 `B09` 提供概念验证级的子假设支撑，而不是对正式主假设的全量实证完成声明。**
...
>
> **公开发表纪律**：本文最多支持“初步形态一致”或“概念验证级支持”的表述，不应被包装为大规模经验验证已经完成。
...
| 子假设 | 与 `B09` 的关系 | 当前内容 |
|------|----------------|---------|
| `H1a` | `H1` 的形态支持子假设 | 约束强度与创造力/适应度之间存在倒 U 型关系 |
| `H2a` | `H2` 的探索性子假设 | 动态环境中保留非零偏差通道的策略优于零容忍消除策略 |
| `H4a` | `H4` 编号谱系中的早期子假设 | 认知简化提升探索多样性，但存在效用拐点 |

---

## 二、仿真实验一：约束与创造力的倒 U 型关系（H1a）
...
**H1a 在当前概念验证仿真中得到初步形态支持。它支持 `H1` 的一部分结构直觉，但不等于 `H1` 的完整适应速度命题已获验证。**
...
**H2a 在当前概念验证仿真中获得初步支持。它只能说明“零容忍回退并非总是最优”，不等于 `H2` 的 `FREEZE 率-质量` 倒 U 型关系已经被直接验证。**
...
**H4a 的核心主张（不完备性在一定区间内可作为探索资源）在当前概念验证仿真中获得初步支持。它不等于 `H4` 主假设“独立验证器优于自我验证”已经被当前仿真证明。**
...
| 假设 | 验证结果 | 支持强度 |
|------|---------|---------|
| `H1a` 约束-创造力倒 U 型 | 初步支持 | 概念验证级（对应 `H1` 的形态支持） |
| `H2a` 偏差利用策略优势 | 初步支持 | 概念验证级（对应 `H2` 的探索性支持） |
| `H4a` 认知简化-探索多样性 | 初步支持 | 概念验证级（对应 `H4` 编号谱系的早期子假设） |
...
- 对 `H3`、`H5-H10` 以及 `H1/H2/H4` 的正式主假设版本继续设计新实验或实证接口
