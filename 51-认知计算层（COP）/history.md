...
- 在当时的 `COP` 总架构说明与 `COP.诊断协议.v1.md` 明写 `A-D + 9题` 只是当前场景适配包，不是 `COP` 的全部状态本体。
...
- 当时的活动任务台随后只保留未来场景扩展、损耗估算独立建模与归档残留管理等活动项。

## 活动任务台清空收口

- 已继续检查当时活动任务台中的剩余三项，确认它们都属于未来扩展纪律，而不是当前协议主位阻塞。
...
- 当时该层目录已不再单独保留活动任务台；后续若再扩场景或建模，统一先记入 `history.md`、`B04` 或对应正式协议文。
...
## 2026-04-17 v1.4 R-Series Consolidation

本轮继续压缩 `COP` 根层并列操作手册数量：

- 已将 `COP.R02_场景适配指南.md` 的现行高频内容吸收到 `COP.R01_接触点操作手册.md`
- 已将 `COP.R03_触发器写作训练手册.md` 的训练主轴吸收到 `COP.R01_接触点操作手册.md`
- `R01` 因此升级为同时承担“操作总门 + 场景适配总门 + 训练总门”的现行母文件
- 原 `R02 / R03` 随后退入归档区，不再与当前根层母文件并列显位
